...
def getIntDuration(durationStr):
    # 匹配课程长度
    duration_re = re.findall(r"([a-zA-Z0-9\.]+\s)(year|month|week|yr|yft|semester|term){1}|([a-zA-Z0-9\.]+)(-year|-month|week|yr|yft|semester|term){1}|([0-9\.]+)(yr|yft|\-month){1}",
                             durationStr, re.I)
    # 英文数字对应的数字
    d_dict = {"One": "1",
              "Two": "2",
    ...
    if len(duration_re) > 0:
        ...
        if len(duration_re[0]) > 1:
            if "y" in ''.join(duration_re[0][1]) or "Y" in ''.join(duration_re[0][1]):
                duration_per = 1
            elif "s" in ''.join(duration_re[0][1]) or "S" in ''.join(duration_re[0][1]) or "term" in ''.join(duration_re[0][1]) or "term" in ''.join(duration_re[0][1]):
                duration_per = 2
