...
# 2019.03.18 星期一 数据更新
class UniversityofSouthAustralia_PSpider(scrapy.Spider):
    name = "UniversityofSouthAustralia_P"
    ...
    def parse(self, response):
        links = response.xpath("//div[@id='postgraduate']//div[@class='online-degree-panel medium-up-2 large-up-4']/a/@href").extract()
        # print(len(links))
        links = list(set(links))
        # print(len(links))

        # links = ["http://study.unisa.edu.au/degrees/master-of-medical-sonography-full-fee-paying",
        #          "http://study.unisa.edu.au/degrees/master-of-medical-sonography",
        #          "http://study.unisa.edu.au/degrees/master-of-clinical-pharmacy",
        #          "http://study.unisa.edu.au/degrees/master-of-business-administration-finance",
        #          "http://study.unisa.edu.au/degrees/international-master-of-business-administration-tourism-and-event-management",
        #          "http://study.unisa.edu.au/degrees/master-of-business-administration-marketing/int/",
        #          "http://study.unisa.edu.au/degrees/master-of-architecture",
        #          "http://study.unisa.edu.au/degrees/master-of-social-work", ]
        # for link in links:
    ...
    def parse_url(self, response):
        ...
        if len(span_is) > 0 and "graduate" not in response.url:
            ...
            print("***不是详情页链接")
            links = response.xpath("//tr//td[contains(text(), 'On-campus')]/preceding-sibling::td/a[contains(text(), 'Master of')]/@href|"
                                   "//h2[contains(text(),'Master of')]/following-sibling::*//a/@href").extract()
            if len(links) == 0:
                links = response.xpath(
                    "//a[contains(text(), 'Master of')]/@href").extract()
        ...
        if len(links) > 0:
            for link in links:
                ...
                #     url = "http://study.unisa.edu.au" + link
                # print("url = ", url)
                yield scrapy.Request(parse.urljoin(response.url, link), callback=self.parse_data)
