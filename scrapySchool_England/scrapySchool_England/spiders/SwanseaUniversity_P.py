...
class SwanseaUniversity_PSpider(scrapy.Spider):
    name = "SwanseaUniversity_P"
    # start_urls = ["https://www.swan.ac.uk/postgraduate/taught/"]
    # 接口
    start_urls = ["https://www.swan.ac.uk/sf-widgets/en/course/a-to-z/postgraduate/taught?callback=jQuery172001851942159089104_1529979609089&_=1529979610101"]
        ...
        try:
            ...
            if len(courseDegreeawardStr) != 0:
                ...
                if len(d) != 0:
                    ...
                    item['programme_en'] = ''.join(programme).strip().strip("in").strip()
...
"MSc Social Research Methods": "College of Human and Health Sciences", }
            ...
            if item['modules_en'] == "":
                item['modules_en'] = remove_class(clear_lianxu_space(response.xpath("//div[@id='course-structure']|//div[@id='modules']").extract()))
            ...
            ielts_d = response.xpath("//div[@id='entry-requirements']//*[contains(text(),'IELTS')]//text()").extract()
            # if len(ielts_d) == 0:
            #     print("ielts_d 为空")
            ...
            item['ielts_desc'] = ''.join(ielts_d).strip()
            # print("item['ielts_desc'] = ", item['ielts_desc'])
            ...
            if len(career) == 0:
                career = response.xpath("undefined//*[contains(@id,'careers')]").extract()
            item['career_en'] = remove_class(clear_lianxu_space(career))
            # print("item['career_en'] = ", item['career_en'])

            item['apply_documents_en'] = remove_class(clear_lianxu_space(["""<p>&nbsp;<a name="docs"></a><strong>What documents do I need to send you?</strong></p>
<ul>
<li>A copy of your current or final academic transcripts and their official translations</li>
<li>A copy of your degree certificates (Bachelor and Masters, if applicable) and their official translations</li>
<li>Personal statement outlining the reasons you wish to pursue your receive of study (taught master's candidates only)</li>
<li>Curriculum vitae (taught master's students applying on the basis of experience)</li>
<li>IELTS test report form if English is not your first language</li>
<li>Outline research proposal (research students only)</li>
<li>Copy of your passport (photo page) (non-EU applicants only)</li>
</ul>"""]))
            # print("item['apply_documents_en'] = ", item['apply_documents_en'])

            assessment_en = response.xpath(
                "//div[@id='assessment']|//div[@id='teaching-and-assessment']|"
                "//strong[contains(text(),'Assessment')]/../..").extract()
            ...
            item['require_chinese_en'] = """<h2>Postgraduate Programmes:</h2><div>Candidates must hold a Bachelor Degree, or its equivalent, from a recognised University/College of Higher Education with a minimum of 2.2 Bachelor: 75% (70% for 211/985) awarded with IELTS 6.5 with 6.0 in each part of the test (or equivalent).Non-standard grading systems, such as Hong Kong Baptist University (in conjunction with Beijing Normal University, P R China) and SHU-UTS SILC Business School (awarded by University of Technology, Sydney, Australia):CGPA 2.5/Second Class (Division II) honours</div>"""
            yield item
