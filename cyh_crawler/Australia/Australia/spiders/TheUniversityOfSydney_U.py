...
class TheuniversityofsydneyUSpider(scrapy.Spider):
    ...
    start_urls = []
    for ng in [1,2, 3, 4, 5]:
        full_page = 'https://sydney.edu.au/content/courses/search.result-courses.aoi-.course-level-uc.page' + str(
            ng) + '.html'
        start_urls.append(full_page)
    def parses(self, response):
        urls=response.xpath('//ul/li/a/@href').extract()
        for u in urls:
            if 'bachelor' in u and '-and-' not in u:
                full_url='https://sydney.edu.au'+u
                yield scrapy.Request(url=full_url,callback=self.parse_main)
#     start_urls=['https://sydney.edu.au/courses/courses/uc/bachelor-of-arts-and-bachelor-of-laws.html',
# 'https://sydney.edu.au/courses/courses/uc/bachelor-of-science-and-bachelor-of-laws.html',
# 'https://sydney.edu.au/courses/courses/uc/bachelor-of-economics-and-bachelor-of-laws.html',
# 'https://sydney.edu.au/courses/courses/uc/bachelor-of-engineering-honours-and-bachelor-of-laws.html',
# 'https://sydney.edu.au/courses/courses/uc/bachelor-of-commerce-and-bachelor-of-laws.html',]
    def parse_main(self,response):
        ...
        item['degree_type']='1'
        item['china_score_requirements']='GaoKao Tier1 (see GaoKao INTL student score)'
        ...
        if majorlist != {}:
            ...
            for murl in set(mus):
                ...
                career=mresponse.xpath('//h3[contains(text(),"opportunities")]/following-sibling::div|//h3[contains(text(),"areer Pathways")]/following-sibling::div')
                ca=[]
                for c in career:
                    ca+=etree.tostring(c,method='html',encoding='unicode')
                item['career_en']=remove_class(ca)
                overview = mresponse.xpath('//div[contains(@class,"ubject-area-overview")]')
                ov = []
                for o in overview:
            ...
            if '(' in prog:
                ...
                item['programme_en']=prog.replace('Bachelor of ','').strip()
            yield item