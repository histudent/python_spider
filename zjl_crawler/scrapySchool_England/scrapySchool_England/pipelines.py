...
class ScrapyschoolEnglandPipeline1(object):
    ...
    def process_item(self, item, spider):
        ...
        insert_sql = "insert into tmp_school_uk_ben(university, location, department,programme_en, degree_type, degree_name, " \
                     ...
                     "update_time,ib,alevel) values(%s, %s, %s, %s, %s, %s,%s, %s, %s, %s, %s, %s, %s, %s, %s, " \
        ...
        try:
            ...
            self.cursor.execute(insert_sql, (item["university"], item["location"], item["department"], item["programme_en"],
                                 ...
                                 item["update_time"],item['ib'],item['alevel']))
            self.db.commit()
            print("数据插入成功")
...
class ScrapyschoolEnglandPipeline2(object):
    ...
    def process_item(self, item, spider):
        ...
        insert_sql = "insert into tmp_school_uk_yan(university, location, department,programme_en, degree_type, degree_name, " \
                     "start_date, overview_en, duration, duration_per, modules_en, assessment_en, " \
                     "career_en, tuition_fee_pre, tuition_fee, require_chinese_en," \
                     "ielts_desc, ielts, ielts_l, ielts_s, " \
                     "ielts_r, ielts_w, toefl_code, toefl_desc, toefl, toefl_l, toefl_s, toefl_r, toefl_w, application_open_date, deadline, apply_pre, apply_fee, interview_desc_en, " \
                     "portfolio_desc_en, apply_desc_en, apply_documents_en, apply_proces_en, other, url, gatherer, batch_number, " \
                     "update_time,rntry_requirements,teach_time) values(%s,%s,%s, %s, %s,%s, %s, %s, %s, %s, %s, %s, %s, %s, " \
                     "%s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s,%s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, " \
                     "%s,%s,%s)"
        try:
            if self.count == 1:
            ...
            self.cursor.execute(insert_sql, (item["university"], item["location"], item["department"], item["programme_en"],
                                 ...
                                 item["apply_proces_en"],
                                 item["other"], item["url"], item["gatherer"], item["batch_number"],
                                 item["update_time"],item['rntry_requirements'],item['teach_time']))
            self.db.commit()
            print("数据插入成功")
        except Exception as e:
            self.db.rollback()
    ...
    # 2018/09/04    表学信网
class XuexinwangPipeline(object):
    db = pymysql.connect('localhost', 'root', '123456', 'xuexinwang', charset='utf8')
    cursor = db.cursor()

    def process_item(self, item, spider):
        insert_sql = "insert into xuexinwang_major(url,major,code,relevant_major)VALUES(%s, %s, %s, %s)"
        try:
            self.cursor.execute(insert_sql,
                                (item['url'],item['major'],item['code'],item['relevant_major']))
            self.db.commit()
            print("数据插入成功")
